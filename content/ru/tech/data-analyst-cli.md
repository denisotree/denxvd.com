...
Важно также изучить работу с переменными окружения, перенаправлением потоков (`>`, `<`, `|`), а также приобрести навыки написания простых bash-скриптов для автоматизации задач.
...
`jq` — это один из самых часто используемых инструментов, потому что необходимость получить отформатированный JSON возникает часто, а это, наверное, самый быстрый способ для этого. 
...
- **Агрегация данных**:
Нажмите `Shift+F`, чтобы быстро сгруппировать данные по определенной колонке и посчитать `count` по умолчанию. Другие функции агрегации доступны по нажатию на `+`.

- **Экспорт результатов**:
После обработки данных можно сохранить результат в новый файл, нажав `Ctrl+s`. Можно ввести новое имя файла, или сохранить изменения в том же.
...
Очень важная утилита, чтобы в реальном времени видеть как ваши скрипты и ноутбуки потребляют ресурсы компьютера, а также удобно находить и "убивать" зависшие процессы.
...
Все так же, чтобы можно было делать то, что нам нужно, не выходя из консоли. Я не призываю превращать `vim`, а точнее `neovim`, в полноценную IDE, хотя сам так и поступил. Но даже базовое знание vim позволит вам например превратить столбец строк
...
Такие задачи возникают постоянно, и если у вас под рукой есть быстрый и удобный способ их решить, то почему бы им не воспользоваться.
...
```bash
# Затем выполните (скачаются все файлы из списка):
wget -i urls.txt
```
...
- `--convert-links`: преобразует ссылки для оффлайн-просмотра.
